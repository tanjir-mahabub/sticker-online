...
import {
  extractDAttributeValue,
  generateSVGImageData,
} from '@/components/Utils/NewDieCutFN';
...
type OnDieCutReady = (
  result: React.MutableRefObject<fabric.Canvas | null>
) => void;
...
export const useDieCutEffect = (onDieCutReady?: OnDieCutReady) => {
  ...
  const materialDefault = useAppSelector(
    (state) => state.formValues.materialLastSelected
  );
  ...
  const deletePrevDieCut = (canvas: fabric.Canvas) => {
    const existingObject = canvas
      .getObjects()
      .find((obj) => obj.get('id') === 'dieCutImage');
    if (existingObject) {
  ...
  const debouncedGenerateSVGImageData = debounce(
    async (svgString, grow, backgroundColor, callback) => {
      try {
        const modifiedSVG = await generateSVGImageData(
          svgString,
          grow,
          backgroundColor
        );
  ...
  const dieCutGenerating = useCallback(
    (dAttributeValue: string | null) => {
      const canvas = fabricCanvasRef.current;

      const materials = stickerData?.materials?.length
        ? stickerData.materials
        : materialStore;
      const selectedMaterial = materials.find(
        (material) => material.id === materialDefault
      );
  ...
  const handleSVGOperations = useCallback(
    async (
      ...
      try {
        ...
        if (svgString && grow) {
          ...
          if (modifiedSVG && dAttributeValue) {
            ...
            console.error(
              'Failed to extract D attribute value from the modified SVG'
            );
            dispatch(
              setCanvasProperties({
                isLoading: false,
  ...
  useEffect(() => {
    ...
    const dieCutImage = canvas
      .getObjects()
      .find((obj) => obj.id === 'dieCutImage');
