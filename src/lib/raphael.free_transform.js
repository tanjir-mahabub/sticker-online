...
(function (root, factory) {
	...
	Raphael.fn.freeTransform = function (subject, className, options, callback) {
		var
			ft, i, mapped, timeout,
			paper = this,
		...
		ft.showHandles = function () {
			...
			ft.axes.map(function (axis) {
				...
				ft.handles[axis].disc.node.setAttribute('class', `${className} handle arm axis-` + axis);

				if (axis === "y") {
					ft.handles["y"].disc = paper
						.circle(ft.attrs.center.x, ft.attrs.center.y, "11")
						.attr(ft.opts.attrs);

					ft.handles[axis].disc.node.setAttribute('id', 'circleYcenter');
					ft.handles[axis].disc.node.setAttribute('fill', 'url(#rotateImageFill)');
					ft.handles[axis].disc.node.setAttribute('class', `${className}`);
				}
			});
			...
			if (ft.opts.draw.indexOf('bbox') >= 0) {
				...
				for (i = (ft.opts.scale.indexOf('bboxCorners') || ft.opts.customCorners >= 0 ? 0 : 4); i < (ft.opts.scale.indexOf('bboxSides') === -1 ? 4 : 8); i++) {
					...
					if (handle.isCorner && ft.opts.customCorners) {
						...
						handle.element = paper
							.rect(ft.attrs.center.x, ft.attrs.center.y, ft.opts.size[handle.isCorner ? 'bboxCorners' : 'bboxSides'] * 2, ft.opts.size[handle.isCorner ? 'bboxCorners' : 'bboxSides'] * 2, ft.opts.round)
							.attr(ft.opts.attrs);

						if (handle.isCorner) {
							handle.element.node.setAttribute('class', `${className} handle bbox corner index-` + i + ' axis-' + handle.axis);
						} else {
							handle.element.node.setAttribute('class', `${className} handle bbox side index-` + (i - 4) + ' axis-' + handle.axis);
						}
					}
			...
			if (ft.opts.drag.indexOf('center') !== -1) {
				...
				ft.handles.center.disc.node.setAttribute('class', `${className} handle center`);
			}
