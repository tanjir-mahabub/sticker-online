...
(function (root, factory) {
    ...
    function escapeXML(s) {
        // Check if s is undefined or null
        if (s == null) return '';
        ...
        // Check if s is a string before trying to replace
        if (typeof s !== 'string') return s;

        for (var entity in replace) {
            if (replace.hasOwnProperty(entity)) {
    ...
    */
    function extractStyle(node) {
        return {
            font: {
                family: node.attrs && node.attrs['font-family'] ? node.attrs['font-family'] : null,
                size: node.attrs && node.attrs['font-size'] ? parseInt(node.attrs['font-size']) : null,
                style: null,  // Since 'font-style' is not present in the console.log output, we'll set it to null
                weight: null  // Similarly, 'font-weight' is not present, so setting it to null
            },
            anchor: node.attrs && node.attrs['text-anchor'] ? node.attrs['text-anchor'] : null
        };
    }
    ...
    */
    ...
    var serializer = {
        'circle': function (node) {
            ...
            if (isCircle) {
                ...
                return tag('circle', node.attrs, node.matrix);
            } else {
                // Return an empty string to exclude circle elements
                return '';
            }
        },
        ...
        'rect': function (node) {
            const { data } = node.data();
            const isRect = data === "frame-rect";

            if (isRect) {
                // Return SVG representation of the rectangle
                // console.log(node.attrs);
                return tag('rect', node.attrs, node.matrix);
            } else {
                // Return an empty string to exclude rect elements
                return '';
            }
        },
    ...
    R.fn.toSVG = function (x, y, width, height, strokeColor, removeDieCut) {
        var
            ...
            svg = `<svg style="overflow: hidden; position: relative;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" version="1.1" height="${height}" viewBox="${x + ' ' + y + ' ' + width + ' ' + height}">`;
        ;
